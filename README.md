...
travels is the rig - a point per residue, the joints, the domains - and a REFERENCE
STRUCTURE, which carries everything the model says about itself: neither the
predicted-error matrix nor the per-residue confidence is sent. Both are read back off the
pair by maths the game already runs - the map from each residue's view of every other in
its own frame, the confidence from the lDDT between model and reference - so every
structure has a reference and there is no predicted-versus-not branch anywhere
downstream. It is found by fitting those two readings themselves: a free displacement of
three numbers a residue, started from an inversion of the matrix (classical scaling on
the error as a distance) wobbled to the stated confidence, then walked downhill against
(matrix - the frame reading) and (pLDDT - the lDDT), with a flat-bottomed bond term
holding the backbone. The map is regenerated by the same comparison that draws the live
one, on whichever side holds the fighter, so both players compute it rather than receive
it. A matrix is n by n and a structure is three numbers a residue: GFP 172 KB to 23 KB,
FUS 1,090 KB to 39 KB, with the confidence read back 1 point out on an ordered model and
6 on a disordered one, and the map 0.5 to 3 A of a 31.75 A scale (correlation 0.76 to
0.94).
