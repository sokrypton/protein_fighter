...
the other side when it is chosen, or when a newcomer joins; a guest loads its own. What
travels is the rig - a point and a confidence per residue, the joints, the domains - and
a REFERENCE STRUCTURE for the model's predicted error, which is the one thing too big to
send as itself: a matrix is n by n, 475 KB over a link for a 689-residue fighter and up
to 9 MB to download. It is that matrix inverted as a distance (classical scaling, solved
on at most four hundred residues and spread back along the chain) laid on the model and
knitted back to real bond lengths, and the game regenerates the map from the pair with
the same comparison that draws the live one, on whichever side holds the fighter, so both
players compute it rather than receive it. The confidence travels as itself, a byte a
residue.

Before any of that, a predicted model has its LONG DOUBTFUL STRETCHES CUT OUT - runs of
twenty residues or more below pLDDT 60. They make a bad fighter, a limb of a hundred
residues of noise flailing, and they are the whole of the difficulty in carrying the
model's uncertainty. What is left is a few confident pieces, which is a multi-chain
protein and something the rig already holds together. A folded model loses nothing: GFP,
haemoglobin, Top7 and the crystal structures are untouched, while FUS goes from 526
residues of mostly-disorder to its three real domains. With the hard part gone the
regenerated map is 0.9 A of a 31.75 A scale on GFP, 0.9 on haemoglobin and 2.8 on the
spike, and GFP costs 25 KB against 172.
