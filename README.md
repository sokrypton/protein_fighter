...
for a predicted model, a REFERENCE STRUCTURE rather than its predicted-error matrix: a
structure displaced from the model by as much as the error said, found by inverting the
matrix (classical scaling on the error as a distance, refined). The map is then
regenerated by the same comparison that draws the live one, on whichever side holds the
fighter, so both players compute it rather than receive it. A matrix is n by n and a
structure is three numbers a residue: GFP 172 KB to 25 KB, FUS 1,090 KB to 43 KB, with
the regenerated map 1 to 3.6 A out of a 31.75 A scale (correlation 0.86 to 0.93). A
structure with no predicted error needs no reference: it IS its own reference, and the
floor is zero.
